...
namespace CultMusicLoader.Sounds.Helpers;
public static class SoundHelpers
{
    ...
    internal static Sound MakeSound(string fileName, bool loop = false)
    {
        ...
        if (result != RESULT.OK)
        {
            Plugin.myLogger.LogError($"Error making sound from file {fileName}!");
    ...
    public static void IfErrorPrintWith(this RESULT result, string where)
    {
        if(result != RESULT.OK)
        {
            Plugin.myLogger.LogError($"Sound Error: {result} caught at {where}");
        }
    }
    ...
    internal static string GetPath(string fileName)
    {
        ...
        if (files.Length == 0)
        {
            Plugin.myLogger.LogError($"Error: Couldn't find \"{fileName}\"");
            return null;
        }
        else if (files.Length > 1)
        {
            Plugin.myLogger.LogWarning($"More than one file named \"{fileName}\" found. This may lead to weird behavior.");
        }
